...
setup:
	virtualenv -p `which python3.5` $(WORKON_HOME)/metro
	$(WORKON_HOME)/metro/bin/pip install -U pip wheel
	$(WORKON_HOME)/metro/bin/pip install -Ur requirements/dev.txt
	$(WORKON_HOME)/metro/bin/pip freeze
	npm install
	npm update
	cp metro/settings/local.example.py metro/settings/local.py
	echo "DJANGO_SETTINGS_MODULE=metro.settings.local" > .env
	createdb -E UTF-8 metro
	$(WORKON_HOME)/metro/bin/python manage.py migrate
	if [ -e project.travis.yml ] ; then mv project.travis.yml .travis.yml; fi
	@echo
	@echo "The metro project is now setup on your machine."
...
update:
	$(WORKON_HOME)/metro/bin/pip install -U -r requirements/dev.txt
	npm install
	npm update
