...
Metro is deployed on the following stack.
...
documented on that page, you need to create a new file in ``metro/celery.py`` and then
modify ``metro/__init__.py`` to import that file. You'll also need to customize ``{{
...
``requirements/base.txt`` and installed it)::
    ...
    celery -A metro worker

Additionally you will need to uncomment the ``BROKER_URL`` setting in the project settings::

    # metro/settings/deploy.py
    from .base import *

    # ...
    BROKER_URL = 'amqp://metro_%(ENVIRONMENT)s:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/metro_%(ENVIRONMENT)s' % os.environ
...
the broker by default. The RabbitMQ user will be named ``metro_<environment>`` and the
vhost will be named ``metro_<environment>`` for each environment. It requires that you
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out metro.key 2048
  # Make copy of the key with the passphrase
  cp metro.key metro.key.secure
  # Remove any passphrase
  openssl rsa -in metro.secure -out metro.key
  # Generate signing request
  openssl req -nodes -sha256 -new -key metro.key -out metro.csr
